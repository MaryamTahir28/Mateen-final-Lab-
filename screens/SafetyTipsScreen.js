...
const SafetyTipsScreen = () => {
  ...
  const renderItem = ({ item }) => (
    <View style={[commonStyles.item, { backgroundColor: darkMode ? '#333333' : '#FFFFFF' }]}>
      <Text style={[commonStyles.title, { color: darkMode ? '#FFFFFF' : '#800080' }]}>{item}</Text>
    </View>
  );

  const renderSectionHeader = ({ section: { title } }) => (
    <View style={[commonStyles.header, { backgroundColor: darkMode ? '#800080' : '#800080' }]}>
      <Text style={[commonStyles.headerText, { color: darkMode ? '#FFFFFF' : '#FFFFFF' }]}>{title}</Text>
    </View>
  );

  return (
    <View style={[commonStyles.screen, { backgroundColor: darkMode ? '#121212' : '#FFFFFF' }]}>
      <View style={[commonStyles.titleBar, { backgroundColor: darkMode ? 'pink' : 'pink' }]}>
        <Text style={[commonStyles.titleBarText, { color: darkMode ? '#FFFFFF' : '#FFFFFF' }]}>Safety Tips</Text>
      </View>
      <SectionList
        sections={DATA}
